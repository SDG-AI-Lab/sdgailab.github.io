...
audited_at: "2026-07-15T03:00:00+02:00"
config_version: 3
...
> **Unit File Coverage**: 16.2% (11/68 files) - Critical
> **Integration File Coverage**: 0.0% (0/68 files) - Critical
> **Line Coverage**: 24.06% - Critical
> **Tests**: 48
> **Audited**: 2026-07-15
...
- Eleven automated test files now cover library logic, admin auth, admin login behavior, admin media upload behavior, admin project-form behavior, and admin page security behavior.
- Security-targeted structural coverage remains in good shape for the current frontend hardening scope.
- The reported line coverage dropped because the measurement scope is now broader: coverage collection includes both `src/lib/**/*.ts` and `src/islands/**/*.tsx`.
- This is a more honest baseline, not a regression in tested behavior.
...
| `src/islands/admin/news/NewsFormPage.tsx` | islands | Create/update interaction, validation feedback, and navigation on success |
...
1. **[P1]** Add focused UI tests for adjacent admin surfaces such as `src/islands/admin/news/NewsFormPage.tsx`, `src/islands/admin/shared/SlugField.tsx`, and `src/islands/admin/shared/StatusSelect.tsx` to lift the newly expanded coverage baseline.
