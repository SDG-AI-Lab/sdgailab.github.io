...
audited_at: "2026-07-15T02:45:00+02:00"
config_version: 3
...
delta:
  previous_audit: "2026-07-15T02:35"
...
- Structural coverage is still low overall because there are still no integration, E2E, or accessibility tests.
- Line coverage stayed flat because the current `vitest` coverage include list targets `src/lib/**/*.ts` only.
...
The current unit tests are focused and deterministic. The biggest remaining gap is now integration and accessibility breadth rather than security hardening.

---

## Recommendations

1. **[P1]** Add integration tests around the admin auth, CRUD, and storage workflows because these are now the highest-risk uncovered user paths.
2. **[P1]** Add focused UI tests for adjacent admin form surfaces such as `src/islands/admin/news/NewsFormPage.tsx` and shared helpers to widen the editor-surface safety net.
